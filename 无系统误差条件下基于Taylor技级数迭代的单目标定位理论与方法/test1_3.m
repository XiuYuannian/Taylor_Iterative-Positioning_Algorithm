...
%% ================== 存储：RMSE 与 CRB(1D) ==================
RMSE_ATG = zeros(numel(deta3),1);  % AOA+TDOA+GROA
RMSE_TG  = zeros(numel(deta3),1);  % AOA+TDOA
RMSE_AG  = zeros(numel(deta3),1);  % AOA+GROA
CRB_ATG_1D = zeros(numel(deta3),1);
CRB_TG_1D  = zeros(numel(deta3),1);
CRB_AG_1D  = zeros(numel(deta3),1);

%% ================== 预先计算真值观测（无噪） ==================
[theta_t, beta_t, d_t, rho_t] = obs(u_true, W);
%% ================== 主循环：随 GROA 噪声变化 ==================
for k = 1:numel(sigma_groa_list)
    sigma_groa = sigma_groa_list(k);
    % ====== 3种组合的Q(对角阵）======
...
% ===== RMSE（实线）=====
plot(deta3, RMSE_ATG, '-o', 'LineWidth', 1.8);
plot(deta3, RMSE_TG,  '-s', 'LineWidth', 1.8);
plot(deta3, RMSE_AG,  '-^', 'LineWidth', 1.8);
% ===== CRB（虚线）=====
plot(deta3, CRB_ATG_1D, '--o', 'LineWidth', 1.8);
plot(deta3, CRB_TG_1D,  '--s', 'LineWidth', 1.8);
plot(deta3, CRB_AG_1D,  '--^', 'LineWidth', 1.8);
xlabel('\delta_3');
