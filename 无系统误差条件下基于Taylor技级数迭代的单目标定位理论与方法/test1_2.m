...
%% ================== 存储：RMSE 与 CRB(1D) ==================
RMSE_ATG = zeros(numel(deta2),1);  % AOA+TDOA+GROA
RMSE_TG  = zeros(numel(deta2),1);  % AOA+TDOA
RMSE_AT  = zeros(numel(deta2),1);  % AOA+GROA
CRB_ATG_1D = zeros(numel(deta2),1);
CRB_TG_1D  = zeros(numel(deta2),1);
CRB_AT_1D  = zeros(numel(deta2),1);

%% ================== 预先计算真值观测（无噪） ==================
[theta_t, beta_t, d_t, rho_t] = obs(u_true, W);
%% ================== 主循环：随 TDOA 噪声变化 ==================
for k = 1:numel(sigma_diffD_list)
    sigma_diffD = sigma_diffD_list(k);
    % ====== 3种组合的Q(对角阵）======
    Q_ATG = make_Q_ATG(M, sigma_diffD, sigma_rad, sigma_groa);
    Q_TG  = make_Q_TG(M,  sigma_diffD, sigma_groa);
    Q_AT  = make_Q_AT(M,  sigma_diffD,   sigma_rad); 
    % ====== CRB：在真值处计算（不用MC）======
    CRB_ATG_1D(k) = crb_1d(u_true, W, Q_ATG, "ATG");
    CRB_TG_1D(k)  = crb_1d(u_true, W, Q_TG,  "TG");
    ...
    for mc = 1:MC
        ...
        % ---- 组合2：TG
        z_TG = [z_full(M+M + (1:M-1)); z_full(M+M+M-1 + (1:M-1)) ];
        u_hat_TG = solve_wls(u0, z_TG, W, Q_TG, "TG", maxIter, tol);
        err_TG(mc) = norm(u_hat_TG - u_true);
        % ---- 组合3：AT = [rho; theta; beta]
...
% ===== RMSE（实线）=====
plot(deta2, RMSE_ATG, '-o', 'LineWidth', 1.8);
plot(deta2, RMSE_TG,  '-s', 'LineWidth', 1.8);
plot(deta2, RMSE_AT,  '-^', 'LineWidth', 1.8);
% ===== CRB（虚线）=====
plot(deta2, CRB_ATG_1D, '--o', 'LineWidth', 1.8);
plot(deta2, CRB_TG_1D,  '--s', 'LineWidth', 1.8);
plot(deta2, CRB_AT_1D,  '--^', 'LineWidth', 1.8);
xlabel('\delta_2');
